...
function CartPage() {
  ...
  const totalPrice = data.reduce(
    (sum, auction) => sum + auction.buyItNowPrice * (auction.cartQuantity || 1),
    0
  );
  const totalShippingPrice = data.reduce(
    (sum, auction) => sum + auction.shippingPrice,
    0
  );
    ...
    <div className="mx-4">
      ...
      <div className="flex-col">
        {data.map((auction: CompleteAuction) => (
          <div
            key={auction.id}
            className="border p-4 mb-4 flex flex-col md:flex-row h-48"
          >
      ...
      <div className="flex flex-col items-end mt-4">
        <p className="font-bold text-left mb-1.5">
          Total Item Price: ${totalPrice.toFixed(2)}
        </p>
        <p className="font-bold text-left mb-1.5">
          Tax (8%): ${tax.toFixed(2)}
        </p>
        <p className="font-bold text-left mb-1.5">
          Total Shipping Price: ${totalShippingPrice.toFixed(2)}
        </p>
        <p className="font-bold text-left mb-1.5">
